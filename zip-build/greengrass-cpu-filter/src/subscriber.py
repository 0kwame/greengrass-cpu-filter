...
class MyStreamHandler(SubscribeToTopicStreamHandler):
    ...
    #     try:
    #         if hasattr(event, "binary_message") and isinstance(event.binary_message, BinaryMessage):
    #             payload_bytes = event.binary_message.message
    #             if payload_bytes:
    #                 message = json.loads(payload_bytes.decode())
    #                 self.subscriber.handle_message(message)
    #         else:
    #             # Instead of warning, just log debug
    #             self.subscriber.logger.debug(f"Skipping non-binary message: {type(event)}")
